...
# def TRADES_loss_warm_up(model, x_natural, y, soft_labels, attack, index):
...
#     batch_matching_indices = [index_to_batch_idx[idx] for idx in matching_indices if idx in index_to_batch_idx]
#     batch_unmatched_indices = [index_to_batch_idx[idx] for idx in unmatched_indices if idx in index_to_batch_idx]
    
#     # Filter natural and adversarial examples to only include matching indices
#     x_natural_matching = x_natural[batch_matching_indices]
#     y_matching = y[batch_matching_indices]           
#     x_adv_matching = x_adv[batch_matching_indices]
    
#     # Calculate logits for matching indices
#     logits_natural_matching = logits_natural[batch_matching_indices]
#     logits_adv_matching = logits_adv[batch_matching_indices]
...
#     # Calculate probabilities for matching and unmatched indices
#     nat_probs_ul = nat_probs_ul_all[batch_matching_indices]
#     adv_probs_ul = adv_probs_ul_all[batch_matching_indices]
#     nat_probs_unmatched = nat_probs_ul_all[batch_unmatched_indices]
    
#     soft_labels_unmatched = soft_labels[batch_unmatched_indices]
    
#     # Natural loss with true labels for matching indices
...
#     # Additional loss term: cross-entropy of adversarial examples with true labels for matching indices
#     loss_adv = F.cross_entropy(logits_adv_matching, y_matching)
    
#     # KL divergence loss between unmatched natural probabilities and soft labels
